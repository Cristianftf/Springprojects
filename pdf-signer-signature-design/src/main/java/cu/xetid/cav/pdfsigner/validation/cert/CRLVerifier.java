...
public final class CRLVerifier {
  ...
  ) throws RevokedCertificateException, CertificateRevocationException {
    try {
      Date now = Calendar.getInstance().getTime();
      Exception firstException = null;
      List<String> crlDistributionPointsURLs = getCrlDistributionPoints(cert);
      for (String url : crlDistributionPointsURLs) {
        String crlDistributionPointsURL = mapCrlDistributionPoint(url);
      ...
      String message = "Cannot verify CRL for certificate: " + cert.getSubjectX500Principal();
      log.info(message, ex);
      throw new CertificateRevocationException(message, ex);
    }
  }

  private static String mapCrlDistributionPoint(String url) {
    if (
      url.equals("https://certificados.xetid.cu/externalra-gui/ocsp-crl.xhtml")
    ) return "https://certificados.xetid.cu/externalra-gui/crls/ACXETID_PEM.pem";
    return url;
  }
  ...
  public static void checkRevocation(
    ...
    if (revokedCRLEntry != null && revokedCRLEntry.getRevocationDate().compareTo(signDate) <= 0) {
      ...
      log.info(
        "The certificate was revoked after signing by CRL " +
        crlDistributionPointsURL +
        " on " +
        revokedCRLEntry.getRevocationDate()
      );
  ...
  private static X509CRL downloadCRL(String crlURL)
    ...
    X509CRL crl = SignatureValidationCache.getInstance().crlCache.getIfPresent(crlURL);
    if (crl != null) {
      log.info("CRL cache hit for " + crlURL);
      return crl;
    }

    if (
      crlURL.startsWith("http://") || crlURL.startsWith("https://") || crlURL.startsWith("ftp://")
    ) {
      crl = downloadCRLFromWeb(crlURL);
      log.info("CRL downloaded from " + crlURL);
      SignatureValidationCache.getInstance().crlCache.put(crlURL, crl);
      return crl;
  ...
  public static List<String> getCrlDistributionPoints(X509Certificate cert) throws IOException {
    ...
    if (!(derObjCrlDP instanceof ASN1OctetString)) {
      log.warn(
        "CRL distribution points for certificate subject " +
        cert.getSubjectX500Principal().getName() +
        " should be an octet string, but is " +
        derObjCrlDP
      );
      return new ArrayList<String>();
    }
