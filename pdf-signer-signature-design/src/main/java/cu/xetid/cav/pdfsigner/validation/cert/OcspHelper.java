...
public class OcspHelper {
  ...
  public OCSPResp getResponseOcsp() throws IOException, OCSPException, RevokedCertificateException {
    // verify cache
    String certSerialNumber = certificateToCheck.getSerialNumber().toString(16);
    var item = SignatureValidationCache.getInstance().ocspCache.getIfPresent(certSerialNumber);
    if (item != null) {
      log.info(String.format("OCSP cache hit for %s (%s)", certSerialNumber, ocspUrl));
      ocspResponderCertificate = item.getOcspResponderCertificate();
      return item.getOcspResponse();
    }
    ...
    // cache response
    SignatureValidationCache
      .getInstance()
      .ocspCache.put(
        certSerialNumber,
        new SignatureValidationCacheItem(ocspResponse, ocspResponderCertificate)
      );
    log.info(String.format("OCSP response cached for %s (%s)", certSerialNumber, ocspUrl));
      ...
      if (status instanceof RevokedStatus) {
        ...
        if (revokedStatus.getRevocationTime().compareTo(signDate) <= 0) {
          throw new RevokedCertificateException(
            "OCSP: Certificate is revoked since " + revokedStatus.getRevocationTime(),
            revokedStatus.getRevocationTime()
          );
        }
        log.info(
          "The certificate was revoked after signing by OCSP " +
          ocspUrl +
          " on " +
          revokedStatus.getRevocationTime()
        );
    ...
    try {
      ...
      if (responseCode != HttpURLConnection.HTTP_OK) {
        throw new IOException(
          "OCSP: Could not access url, ResponseCode " +
  ...
  private OCSPReq generateOCSPRequest() throws OCSPException, IOException {
    ...
    Extension responseExtension = new Extension(
      OCSPObjectIdentifiers.id_pkix_ocsp_response,
      false,
      new DLSequence(OCSPObjectIdentifiers.id_pkix_ocsp_basic).getEncoded()
    );
